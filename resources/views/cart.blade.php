                        ...
                        @if (!empty($cart->items))
                            <ul role="list" class="divide-y divide-gray-200 border-b border-t border-gray-200">
                                @foreach ($cart->items as $item)
                                    <li class="flex py-6 sm:py-10 cart-item bg-white"
                                        data-item='@json($item)'>
                                                    ...
                                                    @if ($item['variant_id'])
                                                        <div class="mt-1 text-sm text-gray-500">
                                                            @foreach ($item['attributes'] as $key => $value)
                                                                <p><strong>{{ ucwords($key) }}:</strong> {{ $value }}
                                                                </p>
                                                            @endforeach
                                                        </div>
                                                    @endif
                    ...
                    fetch(quantityInput.dataset.route, {
                        ...
                        .then(data => {
                            if (data.error) {
                                ...
                                cartItem.querySelector('.subtotal').textContent = (item.price * quantityInput.value).toFixed(2);
                                updateCartTotal();
                            }
                        })
                        .catch(error => {
