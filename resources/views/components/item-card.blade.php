...
<div id="productModal-{{ $item->id }}"
    ...
    <div class="modal bg-white rounded-lg shadow-lg w-full max-w-3xl p-6 relative overflow-y-auto max-h-[90vh]">
        <button onclick="closeModal('productModal-{{ $item->id }}')" class="text-xl font-bold absolute top-4 right-4">
            ✕
        </button>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <h3 class="text-xs mt-2">{{ $item->book?->description }}</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-1 mt-2 text-xs">
                ...
                <form>
                        ...
                        <div class="flex gap-3 mt-2">
                            ...
                            <label class="cursor-pointer">
                                <input type="radio"
                                    ...
                                    value="{{ $value }}"
                                    class="hidden peer variation-option"
                                    @if($index===0) checked @endif>
                                <span class="px-3 py-1 border rounded peer-checked:bg-blue-500 peer-checked:text-white">
                                    {{ $value }}
                                </span>
                            </label>
                            @endforeach
                    ...
                    <div class="mt-4">
                        <strong>Stock:</strong>
                        <span id="stock-display">{{ $item->stock ?? '-' }}</span>
                    </div>
                    ...
                    <div class="mt-2">
                        ...
                        <div class="flex items-center space-x-2 mt-2">
                            ...
                            <button type="button"
                                onclick="incrementQuantity('quantity-{{ $item->id }}')"
                                class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-200 hover:bg-gray-300 text-xl font-bold">
                                +
                            </button>
                        </div>
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2">
                        <button type="submit" name="action" value="reserve" class="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 w-full sm:w-auto">
                            Reserve
                        </button>
                        <button type="submit" name="action" value="cart" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 w-full sm:w-auto">
                            Add to cart
                        </button>
                    </div>
                </form>
